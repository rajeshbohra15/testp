	...
	var orderable_layouts = {
		...
		on_ready: function() {
			...
			$( document.body ).on( 'change', '.orderable-table--product-lists input, .orderable-table--product-lists select', function( e ) {
				...
				var default_shortcode_parameters = {
					categories: '',
					layout: 'grid',
					images: 'true'
				};
	...
	var orderable_multiselects = {
		...
		init: function( $selects ) {
			...
			$selects.each( function( index, select ) {
				...
				var $labels = $multi_select.find( '.multi-select-menuitem' ),
		 ...
		 * @param $selects
		 */
		destroy: function( $selects ) {
			var $multi_selects = $selects.siblings( '.multi-select-container' );
...
(function( $, document ) {
	...
	var orderable_onboard = {
		...
		init: {
			 ...
			 * After woo installed.
			 */
			woo_installed: function( event, data ) {
...
jQuery( document ).ready( function() {
	function show_orderable_pointer( id ) {
		var pointer = orderable_pointers.pointers[ id ];
		...
		var options = jQuery.extend( pointer.options, {
			...
			skip: function() {
				var active_pointers = document.querySelectorAll( '.wp-pointer.orderable-pointer' );
			...
			buttons: function( event, t ) {
				var next = pointer && pointer.next && orderable_pointers.pointers[ pointer.next ] ? orderable_pointers.i18n.next : orderable_pointers.i18n.close,
					button = jQuery( '<a class=\"button button-primary\" href=\"#\">' + next + '</a>' ),
					wrapper = jQuery( '<div class=\"wc-pointer-buttons\" />' );

				var skip = orderable_pointers.i18n.skip,
	...
	var orderable_timings = {
		...
		on_ready: function() {
			...
			$( document.body ).on( 'change', '.orderable-enable-day', function( e ) {
				...
				$( document ).trigger( 'orderable-destroy-multiselects', { selects: $day_selects } );
				$( document ).trigger( 'orderable-init-multiselects', { selects: $day_selects } );
			} );
			...
			$( document.body ).on( 'change', '[name*="\\[store_general_services\\]"]', function( e ) {
				...
				var $selected_services = $( '[name*="\\[store_general_services\\]"]:checked' );
				...
				if ( $selected_services.length <= 0 ) {
					...
					var active_service = $selected_services.eq( 0 ).val(),
				...
				var $same_as_delivery_checkbox = $( '#orderable_settings_store_general_service_hours_pickup_same' ),
	...
	var orderable_triggers = {
		...
		on_ready: function() {
			...
			$( document.body ).on( 'click change', '[data-orderable-trigger]', function( e ) {
				...
				$( document.body ).trigger( 'orderable-' + trigger, { trigger_element: $trigger_element } );
			} );
			...
			$( document.body ).on( 'orderable-new-row', function( e, data ) {
				var $button = $( data.trigger_element ),
					...
					new_index = index + 1,
					row_html = $row[ 0 ].outerHTML.replace( /\[\d+\]/gm, '[' + new_index + ']' ).replace( /data-orderable-index="\d+"/gm, 'data-orderable-index="' + new_index + '"' );

				$body.append( row_html );

				var $new_row = $body.find( '.orderable-table__row--repeatable:last-child' );

				// Reset inputs.
				$new_row.find( 'input' ).not( 'input[type="checkbox"]' ).val( '' );

				// Reset datepickers.
				var $datepickers = $new_row.find( '.hasDatepicker' );
				...
				$( document ).trigger( 'orderable-init-multiselects', { selects: $body.find( '.orderable-select--multi-select' ) } );
		 ...
		 * Add last row class.
		 */
		add_last_row_class: function() {
			var $tables = $( '.orderable-table' ),
