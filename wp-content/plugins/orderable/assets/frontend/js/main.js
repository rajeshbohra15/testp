	...
	var orderable_accordion = {
		...
		watch: function() {
			...
			$( document.body ).on( 'orderable-scrollbar.created', function( e, args ) {
				var $active_accordion = $( '.orderable-drawer .' + orderable_accordion.vars.classes.link_active );

				if ( $active_accordion.length <= 0 ) {
					return;
				}

				var $scroll_content = args.content,
...
(function( $, document ) {
	...
	var orderable_drawer = {
		 ...
		 */
		debounce: function (func, timeout = 700) {
			let timer;
		 ...
		 * @param {Event} event 
		 */
		allow_only_numbers: function (event) {
		...
		on_ready: function() {
			...
			$( '.orderable-drawer__cart' ).on('input', '.orderable-quantity-roller__quantity', orderable_drawer.debounce(orderable_drawer.on_change_quantity));
		...
		watch: function() {
			...
			var drawer = document.querySelector( 'body:not( .rtl ) .orderable-drawer' );
			var drawer_rtl = document.querySelector( 'body.rtl .orderable-drawer' );
		...
		cart: {
			...
			click_increase_decrease_quantity: function( e, $button ) {
				var product_id = $button.attr( 'data-orderable-product-id' ),
				...
				this.currentRequest = jQuery.post(
					orderable_vars.ajax_url,
					data,
					(function( response ) {
						if ( ! response ) {
						...
						if (data.quantity === $quantityElement.attr('data-orderable-updating-quantity')) {
			...
			handle_quantity_change_by_button: function (e, $button) {
...
(function( $, document ) {
	...
	var orderable_products = {
		 ...
		 * @param $element
		 */
		view_product: function( event, $element ) {
			var product_id = $element.data( 'orderable-product-id' ),
				args = { action: 'product-options' };
		 ...
		 * @param $element 
		 */
		edit_cart_item: function(event, $element) {
		 ...
		 * @param $element 
		 */
		update_cart_item: function(event, $element) {
			const cart_item_key = $element.data('orderable-cart-item-key');
		 ...
		 * @param callback
		 */
		 get_cart_item_options: function( args, callback ) {
		 ...
		 * @param callback
		 */
		 update_cart_item_options: function( args, callback ) {
			if ( typeof args.cart_item_key === 'undefined' ) {
				return;
			}
		...
		init_product_options: function( event, args ) {
			...
			var selectors = '.orderable-drawer .orderable-product__options input, .orderable-drawer .orderable-product__options select, .orderable-product__options textarea';
			...
			var debounced_update_button_state    = orderable_products.debounce( orderable_products.update_button_state, 500 );
			var debounced_product_options_change = orderable_products.debounce( orderable_products.product_options_change, 500 ); 
		...
		product_options_change: function( $options ) {
			var $add_to_order_button = $( '.orderable-drawer .orderable-product__add-to-order, .orderable-drawer .orderable-product__update-cart-item' ),
		...
		check_variation: function( $options ) {
			...
			var variations = $product.find( '.orderable-product__variations' ).text();

			variations = variations ? JSON.parse( variations) : '';
			    
			var selected_options    = orderable_products.serialize_object( $options ),
		...
		update_button_state: function () {
			...
			setTimeout( function () {
				var $button = $( '.orderable-drawer .orderable-product__add-to-order, .orderable-drawer .orderable-product__update-cart-item' ),
		 ...
		 * @returns {Object}
		 */
		add_unchecked_checkbox_fields: function ( inputs ) {
			jQuery( '.orderable-product-fields-group :input[type="checkbox"]:not(:checked)' ).each(
	...
	var orderable_tabs = {
		...
		watch: function() {
			...
			$(document).on( 'click', '.' + orderable_tabs.vars.classes.tab_arrow_right, function( e ) {
				...
				$list.animate( { scrollLeft: $list.scrollLeft() + ( $wrapper.width() * 0.5 ) } );
			} );
...
var orderable_timings = {}; // Make this global so pro modules can access it.
...
(function( $, document ) {
	...
	orderable_timings = {
		...
		on_ready: function() {
			...
			orderable_timings.set_date_and_time();
		},
		 ...
		 * they need to.
		 */
		set_date_and_time: function() {
			var $date = document.querySelector( '#orderable-date' );
			var date_option = '';
			var date_value = '';
			if ( $date ) {
				...
				if ( date_option ) {
					...
					if ( ! date_value && $date.options[ $date.selectedIndex + 1 ] ) {
		 ...
		 */
		restore: function() {
			var timings = orderable_timings.get_timings();

			if ( ! timings || ! timings.date ) {
				return;
			}

			$( '.orderable-order-timings__date' ).val( timings.date );
			$( '.orderable-order-timings__date' ).change();

			if ( ! timings.time ) {
				return;
			}

			$( '.orderable-order-timings__time' ).val( timings.time );
			$( '.orderable-order-timings__time' ).change();
		},
