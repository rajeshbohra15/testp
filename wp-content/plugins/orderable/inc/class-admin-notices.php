...
class Orderable_Admin_Notices {
	...
	public static function dismiss() {
		...
		$action = filter_input( INPUT_GET, 'orderable_action', FILTER_SANITIZE_STRING );
		...
		// Get notice.
		$name = filter_input( INPUT_GET, 'orderable_notice', FILTER_SANITIZE_STRING );
		...
		foreach ( $notices as $notice ) {
			...
			<div class="notice notice--orderable" style="border-left-color: #7031F5;">
				...
				<?php if ( $notice['dismissable'] ) { ?>
					<p>
						<a href="<?php echo esc_url( add_query_arg( array( 'orderable_action' => 'dismiss_notice', 'orderable_notice' => $notice['name'] ) ) ); ?>"><?php _e( 'Dismiss Notice', 'orderable' ); ?></a>
					</p>
				<?php } ?>
			</div>
			<?php
