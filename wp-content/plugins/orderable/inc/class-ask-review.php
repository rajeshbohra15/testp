...
class Orderable_Ask_Review {
	...
	public static function dismiss_notice() {
		...
		$_nonce                          = filter_input( INPUT_GET, '_nonce' );
	...
	public static function show_notice() {
		...
		if ( isset( $_SERVER['HTTP_HOST'] ) && isset( $_SERVER['REQUEST_URI'] ) ) {
			...
			$dismiss_url  = esc_url( $current_url );
		}
		...
		if ( self::should_display_notice() ) {
			...
			<div class="notice notice-warning is-dismissible notice-orderable-ask-review" style="border-left-color: #4233B6;">
				<h4 style='margin-bottom: 10px;'>You have processed <?php echo esc_html( self::$order_count_required ); ?>+ orders with Orderable 🥳</h4>
				<p>You have been using Orderable for a while. If you are enjoying Orderable, please help us by leaving a review on WordPress.org </p>
				<p>
					<a href='<?php esc_attr( $review_url ); ?>' target='_blank' class="button button-primary">Rate Now</a>
					<a href='<?php echo esc_attr( $dismiss_url ); ?>' class="button button-default">Dismiss Forever</a>
				</p>
			</div>
			<?php
		}
