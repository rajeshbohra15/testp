...
class Orderable_Settings {
	...
	public static function init_onboarding() {
		...
		$slides = apply_filters( 'orderable_onboarding_slides', array(
			'welcome'      => array(
	...
	public static function save_onboard_settings( $result, $fields ) {
		...
		$from = array(
			'hour'   => filter_var( $fields['orderable_settings']['iconic_onboard_open_hours']['from']['hour'], FILTER_SANITIZE_STRING ),
			'minute' => filter_var( $fields['orderable_settings']['iconic_onboard_open_hours']['from']['minute'], FILTER_SANITIZE_STRING ),
			'period' => filter_var( $fields['orderable_settings']['iconic_onboard_open_hours']['from']['period'], FILTER_SANITIZE_STRING ),
		);
		$to   = array(
			'hour'   => filter_var( $fields['orderable_settings']['iconic_onboard_open_hours']['to']['hour'], FILTER_SANITIZE_STRING ),
			'minute' => filter_var( $fields['orderable_settings']['iconic_onboard_open_hours']['to']['minute'], FILTER_SANITIZE_STRING ),
			'period' => filter_var( $fields['orderable_settings']['iconic_onboard_open_hours']['to']['period'], FILTER_SANITIZE_STRING ),
		);
	...
	public static function save_category_fields( $term_id, $tt_id = '', $taxonomy = '' ) {
		...
		if ( ! is_null( $visibility ) && 'product_cat' === $taxonomy ) {
			update_term_meta( $term_id, 'visibility', sanitize_text_field( $visibility ) );
		}
	}
	...
	public static function admin_pointers() {
		...
		if ( $screen_id === 'orderable_page_orderable-settings' ) {
			$pointers['orderable-store-settings'] = array(
				'target'  => '.wpsf-nav a[href="#tab-store"]',
				'next'    => 'orderable-layout-builder',
				'options' => array(
					'content'  => '<h3>' . esc_html__( 'Set Up Your Store', 'orderable' ) . '</h3>' .
					              '<p>' .
					              esc_html__( "Use the Store Settings tab to configure your store's opening hours, delivery/pickup schedule, and holidays.", 'orderable' ) .
			...
			$pointers['orderable-addons'] = array(
				'target'  => '#toplevel_page_orderable a[href="admin.php?page=orderable-product-addons"]',
				'options' => array(
					'content'  => '<h3>' . esc_html__( 'Product Addons', 'orderable' ) . '</h3>' .
					              '<p>' .
					              esc_html__( 'Create product options which can be conditionally applied to the products in your store.', 'orderable' ) .
					              ' <a href="https://orderable.com/product-addons?utm_source=orderable&utm_medium=plugin&utm_campaign=pointer" target="_blank">' . esc_html__( 'Learn more' ) . '</a>.' .
					              '</p>',
					'position' => array(
						'edge'  => 'left',
						'align' => 'left',
