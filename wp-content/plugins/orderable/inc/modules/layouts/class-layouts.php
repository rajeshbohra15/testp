...
class Orderable_Layouts {
	...
	public static function admin_columns_content( $column_name, $post_ID ) {
		...
		} else if ( 'php_function' === $column_name ) {
	...
	public static function save_meta( $post_id ) {
		...
		/**
		 * Filter the layout settings to be saved.
		 *
		 * @since 1.0.0
		...
		$layout_settings = apply_filters(
			...
			array(
				...
				'layout'     => filter_input( INPUT_POST, 'orderable_layout', FILTER_SANITIZE_STRING ),
				'images'     => 'yes' === filter_input( INPUT_POST, 'orderable_images', FILTER_SANITIZE_STRING ),
				'card_click' => filter_input( INPUT_POST, 'orderable_card_click', FILTER_SANITIZE_STRING ),
			)
		);
	 ...
	 * @return mixed|void
	 */
	public static function get_layout_sections_field( $layout_settings ) {
		ob_start();
		?>
		<?php echo Orderable_Helpers::get_pro_button( 'max-orders' ); ?>
		<?php

		return apply_filters( 'orderable_layout_sections_field', ob_get_clean(), $layout_settings );
	}
	...
	public static function orderable_shortcode( $args, $content = '', $name = '' ) {
		...
		$args['images'] = (boolean) json_decode( strtolower( $args['images'] ) );
	...
	public static function get_layout_settings( $layout, $string = false ) {
		...
		$layout_settings = self::get_layout_defaults( $layout->ID );
