 ...
 */
	...
	public static function product_add_to_cart( $args ) {
		...
		extract( $args );
		$cart_icon = !empty($cart_icon) ? $cart_icon : "wpcafe-cart_icon";
		$customization_icon = !empty($customization_icon) ? $customization_icon : "wpcafe-customize";
		// qr code parameter.
			...
			&& $product->is_in_stock() == true :
				if( $cart_button=='on' || $cart_button =='yes' ) {
					if ( class_exists('\Wpcafe_Pro') ) {
						apply_filters("wpcafe/shortcode/variation", $product, $customize_btn, $widget_id, $customization_icon );
					} else {
					ob_start();
					?>
						<div class="wpc-add-to-cart">
							<a href="<?php echo esc_url( $product->get_permalink() ); ?>" class="wpc-btn">
			...
			case ($product->get_type() == 'simple' ) &&
				...
				if ( class_exists('\Wpcafe_Pro') ) {
					...
					if ( ( !empty($wpc_addons) && count($wpc_addons) > 0) ) {
						apply_filters("wpcafe/shortcode/simple", $product, $customize_btn, $widget_id , $customization_icon );
					}else {
						?>
							<div class="wpc-add-to-cart">
